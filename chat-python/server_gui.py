...
def send_receive_client_message(client_connection, client_ip_addr):
    ...
    while True:
        ...
        if data == "exit": break
            ...
            msg = data.split(" ")
            if len(msg) < 3:
                client_connection.send("Invalid message".encode())
            else:
                recipient = msg[1]
                message = " ".join(msg[2:])
                if recipient in clients_names:
                    idx = clients_names.index(recipient)
                    recipient_conn = clients[idx]
                    server_msg = str(client_name + " -> " + message)
                    recipient_conn.send(server_msg.encode())
                else:
                    client_connection.send("Invalid recipient".encode())     
        else:
            client_msg = data
